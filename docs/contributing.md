# Contributing to senaite.diagnosis

Third-party contributions are essential for keeping `senaite.diagnosis` continuously 
improving. We simply cannot access the huge number of platforms and myriad 
configurations for running `senaite.diagnosis`. We want to keep it as easy as 
...
The following is a set of guidelines for contributing to `senaite.diagnosis`, which is 
...
bugs](https://github.com/senaite/senaite.diagnosis/issues)? Do you have a
suggestion for improvement? Then by all means please [submit a new
issue](https://github.com/senaite/senaite.diagnosis/issues/new), and do not
hesitate to comment on existing [open
issues](https://github.com/senaite/senaite.diagnosis/issues).
...
The list of [open issues](https://github.com/senaite/senaite.diagnosis/issues)
...
### Preliminaries
   ...
   2. [Fork senaite.diagnosis from GitHub](
   https://github.com/senaite/senaite.diagnosis/fork). This will copy the whole
   `senaite.diagnosis` repository to your personal account.
   ...
   the actual username of your GitHub account):
          ...
          $ git clone https://github.com/YOUR_USERNAME/senaite.diagnosis

   4. Your forked repository https://github.com/YOUR_USERNAME/senaite.diagnosis will 
   receive the default name of `origin`. You can also add the original 
   `senaite.diagnosis` repository, which is usually called `upstream`:

          $ cd senaite.diagnosis
          $ git remote add upstream https://github.com/senaite/senaite.diagnosis.git
...
```
$ git remote -v
origin    https://github.com/YOUR_USERNAME/senaite.diagnosis.git (fetch)
origin    https://github.com/YOUR_USERNAME/senaite.diagnosis.git (push)
upstream  https://github.com/senaite/senaite.diagnosis.git (fetch)
upstream  https://github.com/senaite/senaite.diagnosis.git (push)
```
...
Alternatively, you can merge `senaite.diagnosis`'s `master` into your branch:
...
request](https://github.com/senaite/senaite.diagnosis/compare/). Head to your
